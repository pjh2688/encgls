...
public class PageMaker {
	...
	public void prevnext(int pageNum) {
		...
		if (pageNum == 1) { //  page번호가 1페이지일 경우 
			setPrev(false); // 첫번째 페이지에선 prev 화살표가 있으면 안되니 false
			setNext(true); // 다음 페이지로 가는 화살표 next는 있어야되니 true
		} else if (getCurrentBlock() == getLastBlock()) { // 현재 페이지 블록이 마지막 페이지 블록일 경우
			setPrev(true);
			setNext(false);
		} else { // 저 위의 두가지 조건이 아니라면
			setPrev(true);
			setNext(true);
		}
	}
