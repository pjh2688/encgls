...
function showArticle(articleUid) {
	...
	pageContext.setMainPage('/admin/02/01PA', json);
}
...
function downloadFile(path, original) {
	...
	ajax_download("/admin/02/01FD", json);
}
...
function gotoPage(page, form) {
	...
	ajaxCallBackJsonPost("/admin/02/01AL", jsonObj, function(response) {
...
function editArticle(articleUid) {
	location.href = "/admin/02/01PE?bbs_noticeuid=" + articleUid;
}
...
function deleteArticle(articleUid) {
	if (confirm("해당글을 삭제할까요?")) {
		...
		ajaxCallBackJsonPost("/admin/02/01AD", json, function(response) {
